...
while i < len(line):
    ...
    while i < len(line):
        if line[i] in small:
            items[j] = items[j]+line[i]
            i += 1
        else:
            break
    while i < len(line):
        if line[i] not in small:
            i += 1
        else:
            break
    j += 1
...
while i < len(line_2):
    ...
    if big_seq <= 2:
        big_seq = 0
        continue
    items.append(line_2[i - big_seq:i - 2])
...
max_seq = ""
for i in range(9):
    digits_re = re.compile(f"{i}+")
    for digit_seq in digits_re.findall(numstr):
        max_seq = digit_seq if len(digit_seq) > len(max_seq) else max_seq

print(f"Самая длинная последовательность одинковых цифр: {max_seq}")
